...
function FormField({ label, value, placeholder, type = "text", full = false, mono = false }: {
  ...
  return (
    <div className={full ? "col-span-2" : ""}>
      <label className="block mb-1.5" style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)" }}>{label}</label>
      <input
        type={type}
        defaultValue={value}
...
function GenderToggle() {
  ...
  return (
    <div>
      <label className="block mb-1.5" style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)" }}>성별</label>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-4" style={{ scrollbarWidth: "none", background: "var(--surface-page, #eef1f4)" }}>
        ...
        >
          <div className="flex items-center gap-4">
            <div
              ...
              style={{ background: "var(--accent-gradient)", color: "#fff", fontSize: "var(--font-size-sm)" }}
            >
              한
            </div>
            <div className="flex items-center gap-2 min-w-0">
              <span style={{ color: "var(--muted-foreground)", fontSize: "var(--font-size-sm)" }}>한미채 · hmc0810</span>
              <span style={{ width: 1, height: 14, background: "var(--border)", display: "inline-block" }} />
              <span style={{ fontSize: "var(--font-size-sm)", fontWeight: 600, color: "#1e2130" }}>N26431021</span>
              <MemberTypeToggle />
            </div>
          </div>
        </div>
                  ...
                  {row.map((cell: any, ci) => (
                    <React.Fragment key={ci}>
                      <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                        <span style={{ fontSize: "var(--font-size-sm)", color: cell.label.startsWith("*") ? "var(--required-color)" : "var(--muted-foreground)", fontWeight: 500 }}>
                          {cell.label}
                        </span>
                      </td>
        ...
        <FormSection title="일반 회원정보" subtitle="16개 항목" icon={<User size={14} />}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <tbody>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--required-color)", fontWeight: 500 }}>* 회원 등록일자</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input type="date" defaultValue="2023-04-15" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>한글명</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="한미채" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
              </tr>
              {/* Row: * 고객 이름/성 — 3칸 입력 */}
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--required-color)", fontWeight: 500 }}>* 고객 이름/성</span>
                </td>
                <td colSpan={3} style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <div className="flex gap-2">
                    <input defaultValue="미채" className="flex-1 rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                    <input defaultValue="Mi-chae" className="flex-1 rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                    <input defaultValue="Han" className="flex-1 rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                  </div>
                </td>
              </tr>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>Nick Name</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input placeholder="닉네임" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>Business Name</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <input placeholder="사업자명" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
              </tr>
              {/* Row: Legal Name | 생년월일 */}
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>Legal Name</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input placeholder="법적 이름" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>생년월일</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>주민등록번호</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <div className="flex gap-1 items-center">
                    <input defaultValue="900822-2······" className="flex-1 rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                    <div className="relative" style={{ flexShrink: 0 }}>
                      <select defaultValue="여" className="rounded outline-none appearance-none" style={{ fontSize: "var(--font-size-sm)", padding: "4px 28px 4px 8px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }}>
                ...
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>EIN Number</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>비자종류</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <div className="relative" style={{ display: "inline-block", width: "100%" }}>
                    <select className="w-full rounded outline-none appearance-none" style={{ fontSize: "var(--font-size-sm)", padding: "4px 28px 4px 8px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }}>
                      <option>() 내국인</option>
                ...
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>체류기간 만료일자</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>주소지</span>
                </td>
                <td colSpan={3} style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <div className="flex gap-2">
                    <input defaultValue="서울특별시 강남구 테헤란로 152, 강남파이낸스센터 23층" className="flex-1 rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                    <button style={{ fontSize: "var(--font-size-sm)", padding: "4px 12px", background: "#f5f6fa", color: "var(--foreground)", border: "1px solid var(--border)", borderRadius: 4 }}>검색</button>
                  </div>
                </td>
              </tr>
              {/* Row: 우편번호 | 연락처 */}
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>우편번호</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="06236" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>연락처</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>휴대폰번호</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="010-2345-6789" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>세금신고번호</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 120, whiteSpace: "nowrap", verticalAlign: "top", paddingTop: 8 }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>메모</span>
                </td>
                <td colSpan={3} style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <textarea
                    ...
                    style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }}
                    onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }}
                    onBlur={(e) => { e.target.style.background = "var(--input-background)"; }}
                  />
                </td>
        ...
        <FormSection title="거래은행 정보" icon={<CreditCard size={14} />}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <tbody>
              ...
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>은행명</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="신한은행" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 80, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>계좌번호</span>
                </td>
                <td style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                ...
                <td style={{ padding: "4px 12px 4px 0", width: 60, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>예금주</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="김지영" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
              </tr>
              {/* Row: SwiftCode | Branch Number */}
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>SwiftCode</span>
                </td>
                <td colSpan={3} style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="SHBKKRSE" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 60, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>Branch Number</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="0234" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
              </tr>
              {/* Row: 은행동합 거래번호 | 실명계좌 인증 */}
              <tr>
                <td style={{ padding: "4px 12px 4px 0", width: 100, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>은행동합 거래번호</span>
                </td>
                <td colSpan={3} style={{ padding: "4px 12px 4px 0", verticalAlign: "middle" }}>
                  <input defaultValue="88012345" className="w-full rounded outline-none transition-all duration-200" style={{ fontSize: "var(--font-size-sm)", padding: "4px 10px", background: "var(--input-background)", border: "none", color: "var(--foreground)" }} onFocus={(e) => { e.target.style.background = "var(--input-focus-bg)"; }} onBlur={(e) => { e.target.style.background = "var(--input-background)"; }} />
                </td>
                <td style={{ padding: "4px 12px 4px 0", width: 60, whiteSpace: "nowrap", verticalAlign: "middle" }}>
                  <span style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>실명계좌 인증</span>
                </td>
                <td style={{ padding: "4px 0 4px 0", verticalAlign: "middle" }}>
        ...
        <FormSection title="소속 그룹 정보" icon={<Building size={14} />}>
          <div className="flex items-center gap-4">
            <span className="shrink-0" style={{ fontSize: "var(--font-size-sm)", color: "var(--required-color)", fontWeight: 500 }}>* 센터</span>
            <div className="relative flex-1">
              <select
                className="w-full rounded py-1.5 text-sm outline-none appearance-none"
                style={{ background: "var(--input-background)", border: "none", color: "var(--foreground)", paddingLeft: 10, paddingRight: 36, fontSize: "var(--font-size-xs)" }}
              >
                <option>본사</option>
                <option>광주 수완</option>
                <option>서울 강남</option>
            ...
            <span className="shrink-0" style={{ fontSize: "var(--font-size-sm)", color: "var(--muted-foreground)", fontWeight: 500 }}>직급</span>
            <div className="relative flex-1">
              <select
                className="w-full rounded py-1.5 text-sm outline-none appearance-none"
                style={{ background: "var(--input-background)", border: "none", color: "var(--foreground)", paddingLeft: 10, paddingRight: 36, fontSize: "var(--font-size-xs)" }}
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {[
              ...
              <label key={item.label} className="flex items-center gap-1.5 cursor-pointer">
                ...
                />
                <span style={{ fontSize: "var(--font-size-sm)", color: "var(--foreground)" }}>{item.label}</span>
              </label>
            ))}
          </div>
        </FormSection>
        ...
        <div className="flex justify-end pt-2 pb-6">
          <button
            className="rounded font-medium transition-all duration-200"
            style={{ fontSize: "var(--font-size-sm)", padding: "7px 13px", background: "var(--save-btn-bg, #007aff)", color: "#fff", border: "none" }}
          >
            등록/저장
          </button>
        </div>
