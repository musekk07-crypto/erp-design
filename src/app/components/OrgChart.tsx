...
function Card({ label, name, id, date, rank, score, isSelf = false }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, color: LABEL_GRAY, marginBottom: 4 }}>
        {isSelf ? "나" : label}
      </div>
      <div style={{ fontSize: 10.5, fontWeight: 700, color: isSelf ? TEXT_BLUE : "#1e2130", marginBottom: 2 }}>
        {name}({id})
      </div>
      <div style={{ fontSize: 9, color: isSelf ? TEXT_BLUE : "#64748b", marginBottom: 1 }}>{date}</div>
      <div style={{ fontSize: 9, color: isSelf ? TEXT_BLUE : "#64748b", marginBottom: 3 }}>{rank}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: isSelf ? TEXT_BLUE : "#1e2130" }}>{score}</div>
    </div>
  );
}
      ...
      <svg width={svgW} height={svgH} style={{ overflow: "visible", display: "block" }}>
        ...
        <foreignObject x={col3X} y={selfCenterY - 16} width={CARD_W + 2} height={34}>
          <div style={{
            ...
            height: 32, fontSize: 11, fontWeight: 600, color: "#1e2130",
          }}>
            {child.name} ({child.id})
          </div>
        </foreignObject>
